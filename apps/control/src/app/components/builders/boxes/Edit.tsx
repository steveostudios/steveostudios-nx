...
const BoxesEdit: React.FC<Props> = (props) => {
	...
	const recalcItems = (items: { [id: string]: Omit<BoxesItem, "id"> }) => {
		const updatedItems = Object.entries(items)
			.sort(
				(
					prev: [string, Omit<BoxesItem, "id">],
					next: [string, Omit<BoxesItem, "id">]
	...
	const newItem = (items: Partial<BoxesItem>[]) => {
		...
		const startColor = !Object.keys(file.items).length
			? 1
			: Object.entries(file.items).sort(
					(prev: [string, BoxesItem], next: [string, Item]) =>
						next[1].order - prev[1].order
			  )[0][1].color;
		...
		const newItems = items.map((item, i) => {
			...
			const newItem: Omit<BoxesItem, "id"> = {
				name: item.name || "test",
				order: order,
							...
							.map(([id, item], i) => {
								...
								return (
									<Row
										...
										colors={boxesThemes[file.theme - 1].colors} // correct but needs to be .find()
									/>
								);
							})}
					</List>
